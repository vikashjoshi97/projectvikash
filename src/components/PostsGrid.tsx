      ...
      <div className="grid grid-cols-3 gap-1 sm:gap-4 md:gap-6">
        {posts.map((post) => (
          <button
            ...
            className="group relative w-full aspect-square bg-slate-100 overflow-hidden cursor-pointer focus:outline-none focus:ring-2 focus:ring-[#4F46E5] rounded-lg sm:rounded-xl border border-slate-200/60 shadow-xs hover:shadow-md hover:-translate-y-0.5 transition-all duration-200"
          ...
          >
            ...
            {post.category && (
              <div className="absolute top-2 left-2 hidden sm:block opacity-0 group-hover:opacity-100 transition-opacity duration-200 z-10">
                <span className="px-2 py-0.5 rounded-full bg-[#111827]/80 backdrop-blur-md text-[10px] font-semibold text-white tracking-wide uppercase border border-white/20">
                  {post.category}
                </span>
              </div>
            )}
            ...
            <div className="absolute inset-0 bg-[#111827]/40 backdrop-blur-[2px] opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-center justify-center gap-4 sm:gap-7 text-white select-none">
              {/* Likes */}
